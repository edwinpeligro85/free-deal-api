...
export class CustomResponse {
    static SuccessResponse(res: Response, code: HttpStatus, data: any) {
        return res.status(code).json({ success: true, result: data })
    }

    static FailedResponse(res: Response, code: HttpStatus, resource: string, id?: number) {
        return res.status(code).json({ success: false, result: CustomResponse.GetException(code, resource, id) })
    }

    private static GetException(code: HttpStatus, resource: string, id?: number) {
        switch (code) {
            case 400: return CustomResponse.GetBadRequest(resource, id);
            case 401: return CustomResponse.GetUnauthorized(resource);
            case 403: return CustomResponse.GetForbidden(resource);
            case 404: return CustomResponse.GetNotFound(resource);
        }
    }
